...
public static class ReceiptPdfRenderer
{
    ...
    private static void DrawPage(Graphics graphics, ReceiptPdfData data, IReadOnlyList<ReceiptRow> rows, int pageNumber, int pageCount)
    {
        ...
        DrawText(graphics, data.OrganizationName, headingFont, muted, Left, 115, Right - Left, 35); DrawText(graphics, "BIÊN LAI THU TIỀN HỌC PHÍ", titleFont, navy, Left, 165, Right - Left, 48);
        DrawText(graphics, $"Mã biên lai: {data.ReceiptCode}    |    Ngày lập: {data.PaymentDate:dd/MM/yyyy HH:mm}", smallFont, muted, Left, 227, Right - Left, 28); graphics.DrawLine(border, Left, 285, Right, 285);
        int y = ContentY;
        foreach (var row in rows)
        {
