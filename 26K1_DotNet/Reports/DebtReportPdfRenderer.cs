...
public static class DebtReportPdfRenderer
{
    ...
    private static void DrawPage(Graphics graphics, DebtReportPdfData data, int pageNumber, int pageCount, IReadOnlyList<PageRow> rows)
    {
        ...
        graphics.DrawString(data.OrganizationName ?? string.Empty, headingFont, muted, new RectangleF(Left, 70, Right - Left, 30)); graphics.DrawString("BÁO CÁO CÔNG NỢ HỌC PHÍ", titleFont, navy, new RectangleF(Left, 112, Right - Left, 44));
