...
| **MoMo Sandbox** | Mock và MoMo triển khai `IPaymentGateway`. MoMo ký HMAC-SHA256, chỉ dùng endpoint thử nghiệm và xác thực phản hồi trước khi ghi nhận. Intent được lưu trước khi gọi mạng; học phí, biên lai và trạng thái hoàn tất được cập nhật trong cùng transaction, chống ghi nhận lại sau restart. Secret lưu bằng DPAPI CurrentUser; test dùng HTTP giả lập, không cần credential. Không hỗ trợ Production, tiền thật, webhook hoặc refund. |
