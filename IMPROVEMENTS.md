...
| **Kiểm soát phiên bản CSDL** | Quản lý schema bằng `PRAGMA user_version`. Ứng dụng kiểm tra cấu trúc tương ứng với version khai báo và thực hiện migration tuần tự từ v1 đến v4; v4 bổ sung `StudentCode` độc lập với khóa chính nội bộ. Các bước migration có rollback và kiểm thử retry khi xảy ra lỗi. |
