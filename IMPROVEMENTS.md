...
| **Cổng thanh toán QR** | VietQR và MoMo Sandbox triển khai `IPaymentGateway`. VietQR tạo mã chuyển khoản và dùng xác nhận thủ công; MoMo ký HMAC-SHA256, truy vấn endpoint thử nghiệm và xác thực phản hồi trước khi ghi nhận. Intent được lưu trước khi gọi mạng; học phí, biên lai và trạng thái hoàn tất được cập nhật trong cùng transaction, chống ghi nhận lại sau restart. Secret MoMo lưu bằng DPAPI CurrentUser; test dùng HTTP giả lập, không cần credential. |
